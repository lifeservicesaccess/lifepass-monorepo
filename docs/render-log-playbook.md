...
| `Policy admin auth mode: configure POLICY_ADMIN_KEY / POLICY_ADMIN_KEYS_JSON or POLICY_ADMIN_JWT_SECRET` | The governance-hardening build now fails closed unless policy admin access is configured. | Set one of `POLICY_ADMIN_KEY`, `POLICY_ADMIN_KEYS_JSON`, or `POLICY_ADMIN_JWT_SECRET` on Render and redeploy. |
| `Policy admin auth mode: invalid config: choose exactly one admin auth mode (POLICY_ADMIN_KEY / POLICY_ADMIN_KEYS_JSON or POLICY_ADMIN_JWT_SECRET)` | Both policy admin auth modes are configured simultaneously, which is now treated as blueprint drift. | Remove either the key-mode env (`POLICY_ADMIN_KEY` or `POLICY_ADMIN_KEYS_JSON`) or the JWT env (`POLICY_ADMIN_JWT_SECRET`), then redeploy. |
| `Durable governance storage: REQUIRE_DURABLE_GOVERNANCE=1; Postgres-backed governance is mandatory but DATABASE_URL / PG_CONNECTION_STRING is not configured` | Production durable governance is enabled but Render is not injecting the Postgres connection string. | Reattach the managed Postgres database to `DATABASE_URL` or `PG_CONNECTION_STRING`, then redeploy. |
...
| HTTP `403` with `Forbidden: invalid policy admin key` | Policy admin endpoint received the wrong admin secret. | Set and use the correct `POLICY_ADMIN_KEY` or the correct secret from `POLICY_ADMIN_KEYS_JSON`. |
| HTTP `503` with `Policy admin auth is misconfigured: choose exactly one admin auth mode` | The deployment is running with both key-mode and JWT-mode policy admin config. | Remove one mode from the deployment env and redeploy. |
